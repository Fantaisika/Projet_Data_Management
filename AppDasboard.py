...
# Construction des graphs
  st.subheader("Suivi des indicateurs sur les transactions", divider='blue')
  ...
  col1,col2,col3 = st.columns(3)
  col1.markdown(f"<h2 style='font-size:15px, color: darkblue;'>Periode: {date_min} au {date_max}:</h2>",
                unsafe_allow_html=True)
  col2.markdown(f"<h2 style='font-size:15px, color: darkblue;'>Nombre de transactions{space*6}{transnumb}</h2>",
                unsafe_allow_html=True)
  col1.markdown(f"<h2 style='font-size:15px, color: darkblue;'>Periode: {date_min} au {date_max}:</h2>",
                unsafe_allow_html=True)
  col3.write(f"Total des montants{space*6}{transamount} USD")

  col1, col2 = st.columns(2)
  col1.markdown(f"<h2 style='font-size:15px;'>Evolution des montants sur la période:</h2>",
                unsafe_allow_html=True)
  col1.area_chart(persondata_filtre[['date','daily_amount$']],
                  x="date", y="daily_amount$", x_label= 'Periode',
                  y_label='Montant USD')

  col2.markdown(f"<h2 style='font-size:15px;'>Evolution des transactions sur la période:</h2>",
                    unsafe_allow_html=True)
